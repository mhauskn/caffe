...
/**
 * Data layer used for learning from Experience tuples. Loads a batch
 * of experience tuples from database then
 * 1) Provides new_states as input in the first forward pass
 * 2) Provides states as input in the second forward pass
 */
template <typename Dtype>
class ExperienceDataLayer : public BasePrefetchingDataLayer<Dtype> {
 public:
  explicit ExperienceDataLayer(const LayerParameter& param)
      : BasePrefetchingDataLayer<Dtype>(param) {}
  virtual ~ExperienceDataLayer();
  virtual void DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top);

  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_EXPERIENCE_DATA;
  }
